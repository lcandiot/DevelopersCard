...
# function Enzyme.onehot(x::Field)
#     N = length(x)
#     ntuple(Val(N)) do i
#         Base.@_inline_meta
#         res = similar(x)
...
Base.similar(x::Field{T,N,L,H,A}, el::Type{ET}) where {T,N,L,H,A, ET} = Field{L,H}(similar(x.data, ET), x.dims)
...
@views function main_diffusion2D(backend=CPU(); nxy = (126, 126))
    ...
    for idx_time in 1:nt
        ...
        time_loop(C, C_old, R_C, q, Δt, k, arch, grid, launch)
        J_enz = Enzyme.jacobian(set_runtime_activity(Forward), Const((C)->time_loop(C, C_old, R_C, q, Δt, k, arch, grid, launch)), C)[1]
        J = ForwardDiff.jacobian(C->time_loop(C, C_old, R_C, q, Δt, k, arch, grid, launch), C)
        @show size(J)
        @show reshape(J, 16, 16) * Array(vec(dC))
        autodiff(
            set_runtime_activity(Forward), 
            time_loop, 
            Const, 
            Duplicated(C, dC), #Vector 
            Const(C_old), 
