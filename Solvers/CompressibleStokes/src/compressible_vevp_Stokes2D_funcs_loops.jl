...
@views function incompressible_viscoelastic_stokes2D()
    ...
    for idx_Time in 1:nt
        ...
        for iter in 1:max_iter
            ...
            compute_divergence_vs!(∇v, vx, vy, dx, dy)
            ...
            update_stresses!(τxx, τyy, τxy, Jxx, Jyy, Jxy, ∇v, vx, vy, ηve, G,  G̃dτ, dx, dy)
...
function compute_divergence_vs!(
    ...
    for idy in 1:ny
        for idx in 1:nx
            ∇v[idx, idy] = @d_dx(vx) / dx + @d_dy(vy) / dy
        end
    end
...
function update_stresses!(
    ...
    for idy in 1:ny
        for idx in 1:nx
            τxx[idx, idy] = ( τxx[idx, idy] + 2.0 * G̃dτ[idx, idy] * ( -1.0 / (2.0 * G[idx, idy]) * Jxx[idx, idy] + @d_dx(vx) / dx - 1.0/3.0 * ∇v[idx, idy]) ) / (G̃dτ[idx, idy] / ηve[idx, idy] + 1.0)
            τyy[idx, idy] = ( τyy[idx, idy] + 2.0 * G̃dτ[idx, idy] * ( -1.0 / (2.0 * G[idx, idy]) * Jyy[idx, idy] + @d_dy(vy) / dy - 1.0/3.0 * ∇v[idx, idy]) ) / (G̃dτ[idx, idy] / ηve[idx, idy] + 1.0)
        end
    end
